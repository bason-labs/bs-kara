export const meta = {
  ...
  description: 'Port 11 missing apps/web features into apps/mobile with per-feature verification',
  phases: [
    { title: 'Analyze', detail: 'Read apps/web source, produce port specs for 11 features' },
    { title: 'Implement', detail: 'Port each feature to apps/mobile (apps/web is read-only)' },
...
const FEATURES = [
  {
    ...
    webFiles: [
      'apps/web/hooks/useSongScore.ts',
    ],
  },
  {
    ...
    webFiles: [
      'apps/web/components/ScoreBlock.tsx',
      'apps/web/components/EndScreenOverlay.tsx',
    ],
  },
  {
    name: 'EmojiLayer',
    ...
    webFiles: [
      'apps/web/components/EmojiLayer.tsx',
    ],
  },
  {
    name: 'useMCPlayer',
    ...
    webFiles: [
      'apps/web/hooks/useMCPlayer.ts',
      'apps/web/hooks/useAIVoice.ts',
    ],
  },
  {
    ...
    webFiles: [
      'apps/web/components/MCAnnouncementOverlay.tsx',
    ],
  },
  {
    name: 'useAutoRandom',
    ...
    webFiles: [
      'apps/web/hooks/useAutoRandom.ts',
    ],
  },
  {
    ...
    webFiles: [
      'apps/web/features/remote/hooks/useInactivityTimeout.ts',
      'apps/web/features/remote/components/SessionExpiredOverlay.tsx',
    ],
  },
  {
    ...
    webFiles: [
      'apps/web/features/remote/hooks/useFullscreenOwnership.ts',
    ],
  },
  {
    name: 'PlayNowButton',
    ...
    webFiles: [
      'apps/web/features/remote/components/PlayNowButton.tsx',
    ],
  },
  {
    name: 'IdleQRCode',
    ...
    webFiles: [
      'apps/web/components/IdleQRCode.tsx',
    ],
  },
  {
    name: 'NeonOrbs',
    ...
    webFiles: [
      'apps/web/features/remote/components/NeonOrbs.tsx',
    ],
  },
]
...
phase('Analyze')
log(`Reading ${FEATURES.length} apps/web features in parallel...`)

const specs = await parallel(FEATURES.map((feature) => () =>
  agent(
    `You are a READ-ONLY agent. Do NOT write any files.

Read the apps/web source files for the feature "${feature.name}" and produce a structured port spec for apps/mobile.
...
2. Return a port spec with these fields:
   ...
   - sourceFiles: the paths you read, relative from ${REPO} (e.g. "apps/web/hooks/useSongScore.ts")
   - targetFiles: [{path, action}] for every apps/mobile file to create or modify.
     Paths relative from ${REPO} (e.g. "apps/mobile/hooks/useSongScore.ts").
...
for (let i = 0; i < FEATURES.length; i++) {
  ...
  phase('Implement')
  await agent(
    `Implement the "${feature.name}" feature in apps/mobile. Port it from apps/web.

HARD CONSTRAINTS — any violation is a bug:
1. apps/web/ is READ-ONLY. Never create, edit, or delete any file under ${REPO}/apps/web/.
2. Only write files under ${REPO}/apps/mobile/.
3. Follow apps/mobile conventions:
...
Implement now. Use Write for new files, Edit for modifications.`,
  ...
  const verifyResult = await agent(
    `Verify apps/mobile compiles and lints cleanly after implementing "${feature.name}".

Run these commands in order:
1. cd ${REPO}/apps/mobile && npx tsc --noEmit 2>&1
2. cd ${REPO}/apps/mobile && npm run lint 2>&1
...
const nextSteps = failedCount > 0
  ? '- Review the failed features above.\n- Fix the reported errors manually in apps/mobile.\n- Re-run the workflow; completed features will be skipped if already verified.'
  : '- All features implemented.\n- Run the full apps/mobile test suite:\n  ```\n  cd apps/mobile && npm test\n  ```\n- Smoke-test the app on a simulator to confirm UI parity with apps/web remote.'
...
# apps/web → apps/mobile Sync Results
