...
// Platform speech: web plays Google TTS through <audio>, mobile through expo-av. Both
// fall back to on-device speech and resolve once the line has finished.
export interface MCVoice {
  speak: (text: string, voiceName?: string) => Promise<void>;
  cancel: () => void;
}

export interface UseMCPlayerArgs {
  ...
  // currentPlaying.id — including the first one observed; the cross-device
  // lock (tryClaimAnnouncementLock) is what suppresses re-announcements on
  // reconnect.
  ...
  // The app's speech implementation (its useAIVoice()).
  voice: MCVoice;
  // Origin of the web app's API routes: '' on web (same origin), the web
  // app's URL on mobile, which has no implicit base.
  apiBase?: string;
}

export interface UseMCPlayerResult {
...
async function fetchLiveMcText(
  ...
  try {
    const res = await fetch(`${apiBase}/api/generate-mc`, {
      method: 'POST',
...
// `parent`, or a timeout, whichever fires first. Hand-rolled because Hermes
// (React Native) has neither AbortSignal.any nor AbortSignal.timeout.
...
// Web: a layout effect, so the iframe gate flips before the browser paints the
// new VideoPlayer. React Native has no paint step to beat, so a plain effect.
const useGateEffect = typeof document !== 'undefined' ? useLayoutEffect : useEffect;
...
export function useMCPlayer({
  ...
  // On web this is a layout effect (see useGateEffect): with a regular effect
  ...
  useGateEffect(() => {
    ...
    // Combined with the timeout via withTimeout so whichever fires first
    // wins.
    ...
    (async () => {
      ...
      if (!text && cp && cp.id === songId) {
        ...
        const liveSignal = withTimeout(liveFetchController.signal, 6000);
        const live = await fetchLiveMcText(
          apiBase,
          cp.title,
          cp.requesterName?.trim() || null,
          liveSignal.signal,
        ).finally(liveSignal.dispose);
        if (cancelled) return;
        text = live;
      }
      if (!text) {
  ...
  }, [ready, isMCEnabled, songId, speak, cancelSpeech, apiBase]);
