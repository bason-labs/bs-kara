...
| 7 | ✅ **De-duplicated web/mobile code where it really was a copy:** `registeredUsers` + `phone` (7a) and `useMCPlayer` (7b, with the platform's speech and API origin passed in) now live in `packages/shared`. Left per-platform on purpose because they differ in platform APIs, not by accident: `useAdMask`, `useAIVoice`, `useCurrentHost` (sync localStorage vs async AsyncStorage), `useRoomGate`, `useSearchHistory`, `useSearchSuggestions` | L | Medium |
