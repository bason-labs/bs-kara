...
| 2 | ⏭️ **Skipped:** extracting `tooling/tsconfig`, `eslint-config` and `vitest-config`. The configs barely overlap (Next, Expo and Node each need their own), so three new packages would add more than they remove. Revisit when a second app of the same kind exists | – | – |
| 3 | ✅ **Web env and config hygiene (reduced scope):** one `SITE_URL` module instead of 3 copies; the Firebase web config lives once in `packages/shared` (`@bs-kara/shared/firebase-config`) and the admin client reuses it; declared the phantom `zod`/`server-only` deps; removed redundant vitest aliases; completed the env list in CLAUDE.md. A zod env schema that fails at startup is **deferred** to when `apps/api` exists: today a missing key disables one feature, and failing at boot would change that behaviour | S | Low |
| 4 | ⏸️ **Deferred:** splitting `packages/shared` into `domain`, `firebase-client`, `i18n` and `client-core`. `shared` is already organised into `lib/` and `hooks/`; four packages only pay off once a server app needs domain logic without the Firebase client, which is when `apps/api` lands | – | – |
| 5 | ✅ **Web server code in one place (`apps/web/server/`)** rather than a new package: Firebase Admin, admin auth, YouTube search, analytics, the subscriptions repo, room-access lookup, MC generation and TTS. The `generate-mc`, `tts` and `room-access` routes are now thin adapters, and this fixes the lib→features and voice→admin import inversions. The `RoomAccessReason` contract moved from the route file to `lib/roomAccess.ts`. When `apps/api` arrives, `apps/web/server` becomes `packages/server-core` in one move | M | Medium |
| 6 | ⏸️ **Deferred:** `packages/contracts` and `packages/test-utils`. Both pay off with a second consumer (`apps/api`, or a second app sharing test factories); until then, contract types live next to their route (e.g. `lib/roomAccess.ts`) | – | – |
| 7 | ✅ **De-duplicated web/mobile code where it really was a copy:** `registeredUsers` + `phone` (7a) now live in `packages/shared`. The shared `useMCPlayer` (7b, #35) was **reverted in #40**, so web and mobile keep their own copies for now. Left per-platform on purpose because they differ in platform APIs, not by accident: `useAdMask`, `useAIVoice`, `useCurrentHost` (sync localStorage vs async AsyncStorage), `useRoomGate`, `useSearchHistory`, `useSearchSuggestions` | L | Medium |
| 8 | ⏸️ **Deferred (add later):** design system: `design-tokens` + `ui` with Storybook | M | Low |
| 9 | ✅ **Web readability:** colocated the last `__tests__/` tests, moved `NeonOrbs`/`OTPInput` to `components/`, removed dead exports (9a); split `RemoteClient` 890→659 lines into `HomeScreen`, `usePlaybackSurface`, `useRoomNotices`, `useHeaderAutoHide` (9b), which also surfaced and fixed an End Party toast bug (#38); split `SearchPanel` 821→628 lines (9c). Per-feature `index.ts` barrels and boundary lint are deferred with the other tooling | M | Low |
