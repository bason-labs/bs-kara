...
import { db, getRoomDataPath, computeScore, type ScoreResult } from '@bs-kara/shared';
...
//
// Persistence at onEnd is the responsibility of the component that wins
// tryClaimScoreLock; this hook only drives the live display.
export function useSongScore(
  ...
  // Reset reactions on song change, enable toggle, or room change.
  useEffect(() => {
    setReactions([]);
  ...
  // serialized via tryClaimScoreLock, so only the lock winner writes.
  // MVP-acceptable.
  useEffect(() => {
