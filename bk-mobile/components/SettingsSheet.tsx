...
export function SettingsSheet({ isOpen, onClose }: SettingsSheetProps) {
  ...
  return (
    <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 100 }}>
      ...
      />
      {/* Sheet */}
      <View style={{ backgroundColor: '#0e1c1c', borderTopLeftRadius: 24, borderTopRightRadius: 24,
        paddingHorizontal: 24, paddingTop: 24, paddingBottom: 40 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20 }}>
          <Text style={{ color: '#e0ffff', fontSize: 20, fontWeight: 'bold', flex: 1 }}>
            {t('settings.title')}
          </Text>
        </View>
        <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 500 }}>
          ...
          <View style={{ backgroundColor: roomData.isAutoRandomMode ? 'rgba(0,139,139,0.08)' : '#152a2a',
            ...
            <TouchableOpacity
              activeOpacity={0.8}
              onPress={() => setAutoRandomMode(!roomData.isAutoRandomMode)}
              style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
                paddingHorizontal: 16, paddingVertical: 14 }}
            >
              <View style={{ flex: 1, marginRight: 12 }}>
                <Text style={{ color: '#e0ffff', fontSize: 14, fontWeight: '600' }}>
              ...
              <Switch
                value={roomData.isAutoRandomMode}
                onValueChange={setAutoRandomMode}
                trackColor={{ false: '#1f3a3a', true: '#008b8b' }}
                thumbColor="#e0ffff"
              />
            </TouchableOpacity>
            ...
            {roomData.isAutoRandomMode && (
              <View style={{ paddingHorizontal: 16, paddingBottom: 16, paddingTop: 8,
                ...
                <Text style={{ color: '#7aa8a8', fontSize: 12, marginBottom: 16 }}>
                  {t('autoRandom.description')}
                </Text>
                <FilterChipRow
                  label={t('autoRandom.genreLabel')}
                  value={roomData.randomFilters.genre}
                ...
                <FilterChipRow
                  ...
                  value={roomData.randomFilters.type}
                ...
                <FilterChipRow
                  ...
                  value={roomData.randomFilters.tone}
          ...
          <View style={{ marginBottom: 20 }}>
            <ToggleRow
              ...
              onValueChange={setMCEnabled}
            />
            <ToggleRow
              label={t('scoring.toggleLabel')}
              hint={t('scoring.toggleHelp')}
              value={roomData.aiScoringEnabled}
              onValueChange={setAiScoringEnabled}
            />
                  ...
                  {MC_VOICE_OPTIONS.map((opt) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => setMcVoice(opt.value)}
                        activeOpacity={0.7}
                        style={{ flex: 1, minWidth: '45%', backgroundColor: '#0e1c1c',
