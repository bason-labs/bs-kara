...
export function SubscriptionsTable({ data, now }: SubscriptionsTableProps) {
  ...
  const columns: DataTableColumn<SubscriptionRecord>[] = [
    ...
    },
    {
      ...
      label: 'Loại',
      render: (row) => (
        <span className="inline-flex rounded-full border border-border bg-bg/40 px-2.5 py-0.5 text-xs text-fg">
          {TYPE_LABEL[row.type]}
        </span>
      ),
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <span
            className={'inline-flex rounded-full px-2.5 py-0.5 text-xs ' + STATUS_CLASS[d]}
          >
            {STATUS_LABEL[d]}
          </span>
        );
